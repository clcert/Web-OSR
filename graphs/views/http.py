...
def device_type_all(request, scan_date):
    scan_date_list = ZmapLog.objects.filter(port=80)
    device80 = HTTPType.objects.filter(port=80, date=scan_date).values('type').order_by('type') \
               .annotate(total=Sum('total')).order_by('-total')[:10]
    device443 = filter_by_name(HTTPType.objects.filter(port=443, date=scan_date).values('type').order_by('type') \
                           .annotate(total=Sum('total')).order_by('-total'), [i['type'] for i in device80], 'type', 'total')
    device8000 = filter_by_name(HTTPType.objects.filter(port=8000, date=scan_date).values('type').order_by('type') \
                            .annotate(total=Sum('total')).order_by('-total'), [i['type'] for i in device80], 'type', 'total')
